      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View className="flex-1 pt-8">
          ...
          <Animated.View className="px-6 py-6" entering={FadeIn.delay(200)}>
            ...
            <Text className="text-base text-white/80 text-center leading-5">
              {getDisplaySubtitle()}
            </Text>
          </Animated.View>
          ...
          <Animated.View className="px-6 mb-6" entering={FadeIn.delay(400)}>
            {features.map((feature, index) => (
              <Animated.View
                key={feature.title}
                className="flex-row items-center mb-4 p-4 bg-white/5 rounded-xl border border-white/10"
              ...
              >
                ...
                <View className="flex-1">
                  <Text className="text-white font-semibold text-base mb-1">{feature.title}</Text>
                  <Text className="text-white/70 text-sm leading-5">{feature.description}</Text>
                </View>
              </Animated.View>
            ))}
          </Animated.View>
          ...
          <Animated.View className="px-6 flex-1 justify-end pb-8" entering={FadeIn.delay(1000)}>
              ...
              <View className="flex-row justify-between items-center">
                <View className="flex-1">
                  <View className="flex-row items-center mb-1">
                    <Text className="text-white font-bold text-base">Yearly</Text>
                ...
                <Text className="text-white font-bold text-lg">{yearlyPrice}</Text>
              </View>
            </TouchableOpacity>
            ...
            >
              <View className="flex-row justify-between items-center">
                <View className="flex-1">
                  ...
                  <Text className="text-white/80 text-xs">3 days free, then billed monthly</Text>
                </View>
                <Text className="text-white font-bold text-lg">{monthlyPrice}</Text>
              </View>
            </TouchableOpacity>
            ...
            {isLoading && (
              <View className="flex-row items-center justify-center mt-3">
                ...
                <Text className="text-white/80 ml-2 text-sm">Processing your subscription...</Text>
              </View>
            )}
            ...
            <View className="mt-3">
              <TouchableOpacity
                ...
                disabled={isLoading}
              >
                <Text className="text-white/60 text-center text-xs underline">
                  Restore Purchases
                </Text>
              </TouchableOpacity>
              ...
              <View className="flex-row justify-center items-center">
                <TouchableOpacity
                  ...
                  className="mr-4"
                >
                  <Text className="text-white/60 text-xs underline">Terms</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => Linking.openURL('https://lunasync.app/privacy')}>
                  <Text className="text-white/60 text-xs underline">Privacy</Text>
                </TouchableOpacity>
              </View>
            </View>
          </Animated.View>
