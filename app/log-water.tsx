    ...
    >
      <View className="flex-1 items-center justify-center px-4">
        ...
        <View className="items-center mb-8">
          <View className="mb-6">
            <CupWithWater fillPercentage={progressPercentage} size={140} isDark={isDark} />
          </View>

          {/* Progress Text */}
          <Text className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-black'}`}>{cupCount} cups</Text>
          <Text className={`text-lg mb-1 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        <View className="flex-row items-center gap-8">
          <TouchableOpacity
            onPress={removeCup}
            className={`w-16 h-16 rounded-full items-center justify-center ${isDark ? 'bg-gray-700' : 'bg-gray-100'}`}
            disabled={cupCount === 0}
          >
            <Minus size={28} color={cupCount === 0 ? '#9CA3AF' : isDark ? '#E5E7EB' : '#374151'} />
          </TouchableOpacity>

          <TouchableOpacity
            onPress={addCup}
            className={`w-16 h-16 rounded-full items-center justify-center ${isDark ? 'bg-pink-900/30' : 'bg-pink-100'}`}
          >
