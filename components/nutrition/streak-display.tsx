...
export default function StreakDisplay({ currentStreak, isLoading = false }: StreakDisplayProps) {
  ...
  return (
    <View className="flex-row items-center">
      ...
      <View
        className={themed(
          'mr-3 bg-orange-100 rounded-xl px-3 py-2 flex-row items-center',
          'mr-3 bg-orange-900/30 rounded-xl px-3 py-2 flex-row items-center'
        )}
      >
        <Flame size={16} color="#F59E0B" />
        <Text
          className={themed(
            'text-orange-700 font-semibold text-sm ml-1',
            'text-orange-400 font-semibold text-sm ml-1'
          )}
        >
          {currentStreak}
        </Text>
