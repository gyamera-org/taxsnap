...
const getCategoryColors = (themed: (light: string, dark: string) => string) => ({
  ...
  science: {
    bg: themed('bg-blue-50', 'bg-blue-900/20'),
    border: themed('border-blue-200', 'border-blue-700'),
    text: themed('text-blue-700', 'text-blue-400'),
  },
  history: {
    bg: themed('bg-purple-50', 'bg-purple-900/20'),
    border: themed('border-purple-200', 'border-purple-700'),
    text: themed('text-purple-700', 'text-purple-400'),
  },
  myth: {
    bg: themed('bg-orange-50', 'bg-orange-900/20'),
    border: themed('border-orange-200', 'border-orange-700'),
    text: themed('text-orange-700', 'text-orange-400'),
  },
  safety: {
    bg: themed('bg-red-50', 'bg-red-900/20'),
    border: themed('border-red-200', 'border-red-700'),
    text: themed('text-red-700', 'text-red-400'),
  },
});
      ...
      <View className={themed('bg-pink-500 rounded-2xl p-6', 'bg-pink-600 rounded-2xl p-6')}>
        <View className="flex-row items-center justify-between">
          <View className="flex-1">
            ...
            <Text className={themed('text-gray-100 text-base', 'text-gray-200 text-base')}>
              {fact.fact}
            </Text>
          </View>
          <TouchableOpacity
            className={themed(
              'bg-white rounded-full px-6 py-3 ml-2',
              'bg-gray-800 rounded-full px-6 py-3 ml-2'
            )}
          ...
          >
            <View className="flex-row items-center">
              <Text
                className={themed(
                  'text-black font-bold text-base mr-2',
    ...
    >
      ...
      <View className="flex-row items-center justify-between mb-4">
        <View className="flex-row items-center">
          <View
            className={themed(
              'bg-white rounded-full p-2 mr-3 shadow-sm',
              'bg-gray-800 rounded-full p-2 mr-3 shadow-sm'
            )}
          >
            <Scan size={20} color="#8B5CF6" />
          </View>
          <Text
            className={themed('text-lg font-bold text-gray-800', 'text-lg font-bold text-gray-200')}
          >
            Daily Beauty Fact
          </Text>
        </View>
        {showRefreshButton && (
          <Pressable
            ...
            className={themed(
              ...
              'bg-gray-800 rounded-full p-2 shadow-sm'
            )}
          >
            <Animated.View style={animatedStyle}>
      ...
      <View className="flex-row items-center mb-4">
        <View
          className={`px-3 py-1 ${categoryColors.bg} ${categoryColors.border} border rounded-full`}
        >
