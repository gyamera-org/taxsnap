-- Remove the problematic trigger and fix onboarding function
...
-- 2. Update the onboarding function to handle account creation/update
...
BEGIN
  ...
  -- 1. Insert or update accounts table with personal info
  INSERT INTO accounts (
    user_id,
    name,
    avatar, -- Keep as 'avatar' since that's what your table has
    date_of_birth,
    ...
    'inactive', -- subscription_status (we'll handle this later)
    'free', -- subscription_plan (default for now)
    NOW(),
    NOW()
  )
  ...
  -- 6. Initialize default reminder settings
  INSERT INTO reminder_settings (user_id, reminder_type, is_enabled, reminder_time)
  VALUES 
    (p_user_id, 'daily_supplements', false, '08:00:00'::time),
    (p_user_id, 'water_intake', false, '09:00:00'::time),
    (p_user_id, 'exercise_reminder', false, '18:00:00'::time),
    (p_user_id, 'meal_logging', false, '12:00:00'::time)
